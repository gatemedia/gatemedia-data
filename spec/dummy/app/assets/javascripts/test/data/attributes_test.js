/* global module:false, test:false, equal:false, ok:false , withFakeAdapter:false, AttributesTest:true */
...
module("Relations: belongsTo", withFakeAdapter(AttributesTest.adapter));
(function () {

  test("retrieve relation once when id is defined", function () {
    AttributesTest.adapter.fakeXHR('GET', 'nodes/42', {"node":{"id":42}});
    ...
    equal(node1, node2);
    equal(AttributesTest.adapter.XHR_REQUESTS.length, 1);
  });

  test("cached relation is not shared across instances", function () {
    AttributesTest.adapter.fakeXHR('GET', 'nodes/36', {"node":{"id":36}});
    AttributesTest.adapter.fakeXHR('GET', 'nodes/37', {"node":{"id":37}});
    ...
    equal(AttributesTest.adapter.XHR_REQUESTS.length, 2);
  });
...
module("Relations: hasMany", withFakeAdapter(AttributesTest.adapter));
(function () {
  ...
  test("does not retrieve relation for new record", function () {
    ...
    equal(AttributesTest.adapter.XHR_REQUESTS.length, 0);
  });
  ...
  test("retrieve relation once for non-empty set", function () {
    AttributesTest.adapter.fakeXHR('GET', 'leafs?ids%5B%5D=301001&ids%5B%5D=301002', {"leafs":[{"id":301001},{"id":301002}]});
