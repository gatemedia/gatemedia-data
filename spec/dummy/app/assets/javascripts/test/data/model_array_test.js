/* global module:false, test:false, asyncTest:false, start:false,
          ok:false, equal:false, deepEqual:false, throws:false,
          withFakeAdapter:false, ModelArrayTest:true */
...
module("Data extension: model array", withFakeAdapter(ModelArrayTest.adapter));
...
asyncTest("new records should be saved after owner", 1, function () {
  ...
  ModelArrayTest.adapter.fakeXHR('POST', 'posts/%@/comments'.fmt(postId), { "comment": { "id": 300100, "post_id": postId } });

  Ember.run(function () {
    post.save().then(function () {
      equal(ModelArrayTest.adapter.XHR_REQUESTS.length, 1);
      start();
    }, function (error) {
